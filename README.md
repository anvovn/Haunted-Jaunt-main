...
Nate Wong: Linear Interpolation
    The main feature I implemented was a dash option for the player. To dash, the player presses space and the character moves forward in the direction they were facing in. I decided to linearly interpolate this movement to look more natural, making the player slow down over time through the duration of the dash. I used a ease out quint curve and multiplied it by the move speed in the line of code that makes the player move. I also used a coroutine to ensure the dashing speed only lasts for a set duration. Lastly, there are serialized variables in the inspector so the developer can customize the length of the dash and rate of movement.
